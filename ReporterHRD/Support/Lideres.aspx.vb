...
Partial Class Mantenimiento_Lideres
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load, TextBoxev.Load
        ...
        If Ano = 2 Then
            Response.AddHeader("REFRESH", "1;Lideres.aspx")
        ...
        timedatea2.Open()
        SQLa2 = "SELECT  Id_Evento, case when count (Id_Evento) < 0 then 1 else 0 end, Tipo_Man from [Sis_HV_Historial]  where (DATEDIFF(Hour, Aud_Time_cierre, ( CONVERT (VARCHAR(10), GETDATE(), 103) + ' ' + CONVERT (VARCHAR(8), GETDATE(), 108) )) >48) and (Estado=2) group by Id_Evento,Tipo_Man  "
        ...
        If Rsa2.Read() Then
            num = Rsa2(0)
            DataNov.InsertCommand = "INSERT INTO Sis_HV_NOVHIST(Evento,  Realiza, NotaRealiza, Date) VALUES (N'" & num & "','Of Sistemas','Cierre automatico del sistema despues de 48 Hrs de ser resuelta la solicitud,  ','" & Hora & "') "
            DataNov.Insert()
            DataHis.UpdateCommand = "UPDATE Sis_HV_Historial SET Sis_HV_Historial.Estado=4, Id_Cierre='Of Sistemas', Obs_Cierre ='Cierre automatico del sistema despues de 48 Hrs de ser resuelta la solicitud' ,Aud_Time_Cierre = N'" & Hora & "'   from [dbo].[Sis_HV_Historial]  where Id_Evento= N'" & num & "'"
            DataHis.Update()

            DataNov.UpdateCommand = "UPDATE Sis_HV_Novedad SET Sis_HV_Novedad.Estado=4 from [dbo].[Sis_HV_Novedad]  where Id_Evento= N'" & num & "'"
            DataNov.Update()
            DataEncuesta.InsertCommand = "INSERT INTO Encuesta(Fecha, Resultado,Tipo_Man, Evento ) VALUES ('" & Hora & "','5','" & Rsa2(2) & "', '" & num & "') "
            DataEncuesta.Insert()
        ...
        DataNov.UpdateCommand = "UPDATE Sis_HV_Novedad SET Sis_HV_Novedad.Limt=1 , Sis_HV_Novedad.FechLimt = N'" & Hora & "' FROM Sis_HV_Novedad INNER JOIN Sis_HV_Historial ON Sis_HV_Novedad.Id_Evento = Sis_HV_Historial.Id_Evento where ((DATEDIFF(Hour, Sis_HV_Novedad.Fech_Sol,  CONVERT (VARCHAR(10), GETDATE(), 103) + ' ' + CONVERT (VARCHAR(8), GETDATE(), 108)) > Sis_HV_Historial.Prioridad)) and  ((Sis_HV_Novedad.Estado%2)<>0) and (Sis_HV_Historial.Prioridad<>0)"
        DataNov.Update()
        ...
        Dim sq6 As String
        Dim timd6 As New SqlConnection("Server=pach\SQLEXPRESS;Database=DBSUPPORT;User ID=sa;Password=Hrd2021Gi;")
        ...
        Dim sq6q As String
        Dim timd6q As New SqlConnection("Server=pach\SQLEXPRESS;Database=DBSUPPORT;User ID=sa;Password=Hrd2021Gi;")
