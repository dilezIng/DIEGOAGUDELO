...
Partial Class Lider3
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load, TextBoxev.Load
        ...
        timedatea2.Open()
        SQLa2 = "SELECT  Id_Evento, case when count (Id_Evento) < 0 then 1 else 0 end, Tipo_Man from [Sis_HV_Historial]  where (DATEDIFF(Hour, Aud_Time_cierre, ( CONVERT (VARCHAR(10), GETDATE(), 103) + ' ' + CONVERT (VARCHAR(8), GETDATE(), 108) )) >48) and Tipo_Man<>5 and (Estado=2) group by Id_Evento,Tipo_Man  "
        ...
        If Rsa2.Read() Then
            ...
            DataNov.Insert()
            DataHis.UpdateCommand = "UPDATE Sis_HV_Historial SET Sis_HV_Historial.Estado=4, Id_Cierre='Of Sistemas', Obs_Cierre ='Cierre automatico del sistema despues de 48 Hrs de ser resuelta la solicitud' ,Aud_Time_Cierre = N'" & Hora & "'   from [dbo].[Sis_HV_Historial]  where Id_Evento= N'" & num & "' and Tipo_Man<>5 "
            DataHis.Update()
