    ...
    <tr >
        <td colspan="4" >
            ...
            <asp:SqlDataSource ID="DataGridindicadores" runat="server" 
                ...
                SelectCommand="SELECT distinct Ind_Principal.IdIndicador,Ind_Principal.CodIndicador,Ind_Principal.NomIndicador ,case when Ind_Principal.IdPadre =1 then 'Padre' else LTRIM(RTRIM(Ind_Principal.CodIndicador)) end as Jerarquia
  FROM Ind_Principal
        ...
        <table class="auto-style11" width="70px">
                ...
                <td class="auto-style40" rowspan="4" align="center">
                    ...
                    <asp:Chart ID="Chart1" runat="server" DataSourceID="Grafica" Width="839px" EnableViewState="True">
                        <Series>
                            ...
                            <asp:Series ChartArea="ChartArea1" ChartType="Column" Name="2019" XValueMember="Mes" YValueMembers="2019" IsValueShownAsLabel="true" CustomProperties="PointWidth=1" Legend="Legend1" MarkerSize="2">
                            </asp:Series>
                  <asp:Series ChartArea="ChartArea1" ChartType="FastLine" Name="2018" XValueMember="Mes" YValueMembers="2018" IsValueShownAsLabel="true" Legend="Legend1" >
                            </asp:Series>
                       <asp:Series ChartArea="ChartArea1" ChartType="FastLine" Name="2017" XValueMember="Mes" YValueMembers="2017" IsValueShownAsLabel="true" Legend="Legend1" >
                            </asp:Series>
                        ...
                        <ChartAreas>
                            <asp:ChartArea Name="ChartArea1" BackColor="Silver">
                                ...
                                <AxisX Crossing="Min" LabelAutoFitStyle="IncreaseFont, DecreaseFont, StaggeredLabels, LabelsAngleStep30, LabelsAngleStep45, WordWrap" IntervalAutoMode="VariableCount">
                                    <MajorGrid Interval="Auto" />
                                    <MajorTickMark Interval="Auto" IntervalOffset="Auto" />
                                    <MinorTickMark IntervalType="Months" />
                                    <LabelStyle Interval="Auto" />
                                    <ScaleBreakStyle Spacing="1" StartFromZero="No" />
                                </AxisX>
                            </asp:ChartArea>
                        </ChartAreas>
                        <Legends>
            ...
            <tr>
                <td colspan="2">
                    ...
                    <asp:SqlDataSource ID="tabla0" runat="server" ConnectionString="<%$ ConnectionStrings:DbBridgeConnectionString %>" SelectCommand="SELECT * FROM (SELECT DISTINCT IdAno AS AÑO, [1] AS ENE, [2] AS FEB, [3] AS MAR, [4] AS ABR, [5] AS MAY, [6] AS JUN, [7] AS JUL, [8] AS AGO, [9] AS SEP, [10] AS OCT, [11] AS NOV, [12] AS DIC, IdItem AS ID FROM Ind_Resultado3 WHERE (IdIndicador = 2) AND (IdItem = 3) ) AS PT PIVOT (max(AÑO) FOR ID IN ([3])) AS ptA">
                    </asp:SqlDataSource>
                    
                    <asp:SqlDataSource ID="Grafica" runat="server" ConnectionString="<%$ ConnectionStrings:DbBridgeConnectionString %>" SelectCommand="SELECT * FROM (SELECT Anual, CASE WHEN Mes = '[1]' THEN 'Ene' WHEN Mes = '[2]' THEN 'Feb' WHEN Mes = '[3]' THEN 'Mar' WHEN Mes = '[4]' THEN 'Abr' WHEN Mes = '[5]' THEN 'May' WHEN Mes = '[6]' THEN 'Jun' WHEN Mes = '[7]' THEN 'Jul' WHEN Mes = '[8]' THEN 'Ago' WHEN Mes = '[9]' THEN 'Sep' WHEN Mes = '[10]' THEN 'Oct' WHEN Mes = '[11]' THEN 'Nov' WHEN Mes = '[12]' THEN 'Dic' END AS Mes, Resultado FROM Ind_Grafica WHERE (IdIndi = 2) GROUP BY Anual, Resultado, Mes) pt PIVOT (max(Resultado) FOR [Anual] IN ([2017],[2018], [2019])) AS ptA">
                    </asp:SqlDataSource>
