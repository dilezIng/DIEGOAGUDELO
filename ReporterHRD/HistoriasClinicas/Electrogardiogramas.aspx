<%@ Page Title="Registro de Enfermeria - Actividades de Enfermeria" Language="VB" MasterPageFile="~/PaginaMaestra.master" AutoEventWireup="false" CodeFile="Electrogardiogramas.aspx.vb" Inherits="PaginaBase" %>
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server"   EnableScriptGlobalization="True">
        ...
        <tr >
            <td colspan="4" 
                ...
                REGISTRO DE ENFERMERIA (Electrocardiograma)</td>
        </tr>
        <tr >
        ...
        <tr >
            <td colspan="4" >
                <asp:GridView ID="GridRegEnfermeria" runat="server" AutoGenerateColumns="False" 
                    DataKeyNames="NumIngreso" DataSourceID="DataGridRegEnf" AllowSorting="True" 
                ...
                <asp:SqlDataSource ID="DataGridRegEnf" runat="server" 
                    ...
                    SelectCommand="SELECT DISTINCT ADNINGRESO.AINCONSEC AS NumIngreso, CASE WHEN GENPACIEN.PACTIPDOC = 1 THEN 'CC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 2 THEN 'CE' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 3 THEN 'TI' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 4 THEN 'RC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 5 THEN 'PA' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 6 THEN 'AS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 7 THEN 'MS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 10 THEN 'CD' ELSE 'AS' END END END END END END END END AS TipoDocPac, LTRIM(RTRIM(GENPACIEN.PACPRIAPE + N' ' + GENPACIEN.PACSEGAPE + N' ' + GENPACIEN.PACPRINOM + N' ' + GENPACIEN.PACSEGNOM)) AS NomPaciente, ADNINGRESO.AINFECING AS FechaIngreso, GENPACIEN.PACNUMDOC AS NumDocPaciente FROM ADNINGRESO INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN HCNREGENF ON ADNINGRESO.OID = HCNREGENF.ADNINGRESO INNER JOIN HCNACTENF ON HCNREGENF.OID = HCNACTENF.HCNREGENF INNER JOIN HCNACTENFD ON HCNACTENF.OID = HCNACTENFD.HCNACTENF WHERE (HCNACTENFD.HCAOBSERV LIKE '%Electrocardiograma%' OR HCNACTENFD.HCAOBSERV LIKE '%Monitoreo Fetal%') AND (HCNACTENFD.HCRHORREG BETWEEN @p0 AND @p1) AND (HCNACTENF.HCTIPACT = 64 OR HCNACTENF.HCTIPACT = 53 OR HCNACTENF.HCTIPACT = 51 OR HCNACTENF.HCTIPACT = 49 OR HCNACTENF.HCTIPACT = 48 OR HCNACTENF.HCTIPACT = 47 OR HCNACTENF.HCTIPACT = 45 OR HCNACTENF.HCTIPACT = 44 OR HCNACTENF.HCTIPACT = 38 OR HCNACTENF.HCTIPACT = 27) ORDER BY FechaIngreso DESC">
                    <SelectParameters>
                        <asp:ControlParameter ControlID="TextFechaIni" Name="p0" PropertyName="Text" />
                        <asp:ControlParameter ControlID="LabelFechaFin" Name="p1" PropertyName="Text" />
                    </SelectParameters>
                </asp:SqlDataSource>
            </td>
        </tr>
        <tr >
            <td colspan="4" >
                ...
                <asp:Panel ID="Panel1" runat="server" BackColor="White" ScrollBars="Vertical" 
                    Width="900px" GroupingText="Procedimientos" Visible="False">
                            ...
                            <tr>
                                <td style="width: 90%; font-size: 10pt;">
                                    ...
                                    <asp:SqlDataSource ID="DataGridDetRegEnf" runat="server" 
                                        ConnectionString="<%$ ConnectionStrings:DGEMPRES99ConnectionString %>" 
                                        
                                        
                                        SelectCommand="SELECT ADNINGRESO.AINCONSEC, HCNACTENFD.OID, HCNACTENFD.GENMEDICO, HCNACTENFD.HCRHORREG, HCNACTENFD.HCNACTENF, HCNACTENFD.HCAOBSERV, HCNACTENFD.OptimisticLockField, HCNACTENFD.HCAHORFIN, HCNACTENFD.HCALITXMIN, GENMEDICO.GMETARPRO, GENUSUARIO.USUDESCRI, ADNINGRESO.OID AS Expr1, CASE WHEN HCTIPACT = 38 THEN 'Canula nasal' WHEN HCTIPACT = 39 THEN 'Ventury' WHEN HCTIPACT = 40 THEN 'Hood' WHEN HCTIPACT = 41 THEN 'Traqueostomia' WHEN HCTIPACT = 42 THEN 'Aspiración Secreciؚón' WHEN HCTIPACT = 43 THEN 'Tubo Orotraqueal' WHEN HCTIPACT = 44 THEN 'Ventilación Mecánica' WHEN HCTIPACT = 45 THEN 'Otros' WHEN HCTIPACT = 67 THEN 'CPAP' END AS NomActividad, HCNACTENF.HCTIPACT AS Expr2 FROM ADNINGRESO INNER JOIN HCNREGENF ON ADNINGRESO.OID = HCNREGENF.ADNINGRESO INNER JOIN HCNACTENF ON HCNREGENF.OID = HCNACTENF.HCNREGENF INNER JOIN HCNACTENFD ON HCNACTENF.OID = HCNACTENFD.HCNACTENF INNER JOIN GENMEDICO ON HCNACTENFD.GENMEDICO = GENMEDICO.OID INNER JOIN GENUSUARIO ON GENMEDICO.GENUSUARIO = GENUSUARIO.OID WHERE (ADNINGRESO.AINCONSEC = @NumIngreso) AND (HCNACTENFD.HCAOBSERV LIKE '%Electrocardiograma%' OR HCNACTENFD.HCAOBSERV LIKE '%Monitoreo Fetal%') ORDER BY HCNACTENFD.HCRHORREG">
                                        <SelectParameters>
                                            <asp:ControlParameter ControlID="GridRegEnfermeria" Name="NumIngreso" 
