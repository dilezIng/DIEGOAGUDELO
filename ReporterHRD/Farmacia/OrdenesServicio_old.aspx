<%@ Page Title="Traza de Ordenes de Servicio" Language="VB" MasterPageFile="~/PaginaMaestra.master" AutoEventWireup="false" CodeFile="OrdenesServicio.aspx.vb" Inherits="PaginaBase" %>
            ...
            <td colspan="5" >
                <asp:GridView ID="GridPrincipal" runat="server" AutoGenerateColumns="False" DataSourceID="DataPrincipal" 
                    ...
                    <Columns>
                        ...
                        <asp:BoundField DataField="HC Medico" HeaderText="HC Medico" 
                            SortExpression="HC Medico" ReadOnly="True" />
                        <asp:BoundField DataField="Pedidas" HeaderText="Pedidas" 
                            SortExpression="Pedidas" ReadOnly="True" />
                        <asp:BoundField DataField="Aplicadas" HeaderText="Aplicadas" 
                            SortExpression="Aplicadas" ReadOnly="True" >
                        </asp:BoundField>
                        <asp:BoundField DataField="Pendiente" 
                            HeaderText="Pendiente" SortExpression="Pendiente" ReadOnly="True">
                        </asp:BoundField>
                ...
                <asp:SqlDataSource ID="DataPrincipal" runat="server" 
                    ...
                    SelectCommand="SELECT ADNINGRESO.AINCONSEC AS Ingreso, GENPACIEN.PACNUMDOC AS Documento, LTRIM(RTRIM(GENPACIEN.PACPRIAPE + N' ' + GENPACIEN.PACSEGAPE + N' ' + GENPACIEN.PACPRINOM + N' ' + GENPACIEN.PACSEGNOM)) AS NomPaciente, INNPRODUC.IPRCODIGO AS Codigo, INNPRODUC.IPRDESCOR AS Medicamento, CONVERT (integer, INNMSUMPA.ISMCANHISCLI) AS 'HC Medico', CONVERT (integer, INNMSUMPA.ISMCANTPEDI) AS 'Pedidas', CONVERT (integer, INNMSUMPA.ISMCANAPL) AS 'Aplicadas', CONVERT (integer, INNMSUMPA.ISMCANPEN) AS 'Pendiente', CONVERT (integer, INNMSUMPA.ISMCANDEV) AS 'Devuelta', GENMEDICO.GMENOMCOM AS MEDICO, SLNORDSER.SOSFECORD AS Fecha_solicitud, HCNFOLIO.HCNUMFOL AS Folio, CASE WHEN HCACODIGO IS NULL THEN 'Urgencias' ELSE HCACODIGO END AS Cama, RIGHT (HPNDEFCAM.HCACODIGO, 2) AS Grupo FROM HCNFOLIO INNER JOIN ADNINGRESO ON HCNFOLIO.ADNINGRESO = ADNINGRESO.OID INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN HCNMEDPAC ON HCNFOLIO.OID = HCNMEDPAC.HCNFOLIO INNER JOIN INNPRODUC ON HCNMEDPAC.INNPRODUC = INNPRODUC.OID INNER JOIN INNMSUMPA ON HCNMEDPAC.OID = INNMSUMPA.HCNMEDPAC INNER JOIN INNCSUMPA ON INNCSUMPA.OID = INNMSUMPA.INNCSUMPA INNER JOIN SLNORDSER ON INNCSUMPA.SLNORDSER = SLNORDSER.OID INNER JOIN GENMEDICO ON INNCSUMPA.GEENMedico = GENMEDICO.OID LEFT OUTER JOIN HPNDEFCAM ON HCNFOLIO.HPNDEFCAM = HPNDEFCAM.OID WHERE (SLNORDSER.SOSFECORD BETWEEN @FechaInicial AND @FechaFinal) AND (HCNMEDPAC.HCSINTRAH = 1) AND (ADNINGRESO.AINCONSEC = COALESCE (NULLIF (@Ingreso, 0), ADNINGRESO.AINCONSEC)) ORDER BY Ingreso, HCNMEDPAC.HCSFECSOL">
                    <SelectParameters>
