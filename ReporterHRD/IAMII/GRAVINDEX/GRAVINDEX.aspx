<%@ Page Title="GRAVINDEX" Language="vb" AutoEventWireup="false" MasterPageFile="~/PaginaMaestra.master" CodeFile="GRAVINDEX.aspx.vb" Inherits="GRAVINDEX" %>
...
<asp:Panel ID="Panelmantenimiento" runat="server" Width="1334px">
    ...
    <table class="auto-style3">
        <tr>
            <td colspan="3" class="auto-style4"><strong>LABORATORIO GRAVINDEX</strong></td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DGEMPRES02ConnectionString %>" SelectCommand="SELECT (select GENPACIEND.PACDIRECCION from GENPACIEND where GENPACIEN.GENPACIEND=GENPACIEND.OID  and PACTIPDIR=1)AS Direccion, ADNINGRESO.AINCONSEC AS Ingreso, GENPACIEN.PACNUMDOC AS Documento, GENPACIENT.PACTELEFONO as Telefono,GENPACIEN.PACPRIAPE + N' ' + GENPACIEN.PACSEGAPE + N' ' + GENPACIEN.PACPRINOM + N' ' + GENPACIEN.PACSEGNOM AS Paciente, ABS(DATEDIFF(yy, GENPACIEN.GPAFECNAC, GETDATE())) AS Edad, GENDETCON.GDENOMBRE + N' (' + GENDETCON.GDECODIGO + N')' AS Entidad , GENMUNICI.MUNNOMMUN AS CODIGOMUNI,LBNORDCAB.ORDCONSEC AS Orden, GENSERIPS.SIPCODIGO as Codigo, GENSERIPS.SIPNOMBRE As Examen, HCNSOLEXA.HCSFECSOL AS Fecha_Solicitud, HCNRESEXA.HCRDESCRIP AS Examen_DESCRIPCIÓN, HCNRESEXA.HCRFECINT AS Fecha_Interpretacion,case when LBNORDDET.EXAESTADO=0 then 'Proceso' when LBNORDDET.EXAESTADO=1 then 'Pendiente' when LBNORDDET.EXAESTADO=2 then 'Validado' when LBNORDDET.EXAESTADO=3 then 'Anulado' end Estado FROM LBNORDDET INNER JOIN LBNORDMUE ON LBNORDDET.OID = LBNORDMUE.LBNORDDET INNER JOIN LBNORDCAB ON LBNORDDET.LBNORDCAB=LBNORDCAB.OID INNER JOIN HCNRESEXA ON LBNORDDET.OID = HCNRESEXA.LBNORDDET INNER JOIN HCNSOLEXA ON HCNRESEXA.HCNSOLEXA = HCNSOLEXA.OID AND HCNRESEXA.OID = HCNSOLEXA.HCNRESEXA INNER JOIN ADNINGRESO ON LBNORDDET.ADNINGRESO = ADNINGRESO.OID AND HCNSOLEXA.ADNINGRESO = ADNINGRESO.OID INNER JOIN LBNEXAMEN ON LBNORDDET.LBNEXAMEN=LBNEXAMEN.OID INNER JOIN GENSERIPS ON LBNEXAMEN.GENSERIPS=GENSERIPS.OID LEFT OUTER JOIN GENUSUARIO ON LBNORDDET.USUVALIDA=GENUSUARIO.OID INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN =GENPACIEN.oid   INNER JOIN GENPACIENT ON GENPACIEN.GENPACIENT=GENPACIENT.OID  INNER JOIN GENDETCON  ON ADNINGRESO.GENDETCON = GENDETCON.OID INNER JOIN GENMUNICI ON GENPACIEN.DGNMUNICIPIO = GENMUNICI.OID WHERE  (HCNSOLEXA.HCSFECSOL BETWEEN CONVERT (DATETIME, @res, 102) AND CONVERT (DATETIME, @res2 + ' 23:59:00', 102)) AND  GENSERIPS.SIPCODIGO IN ('904508','906625','19165')  ORDER BY HCNSOLEXA.HCSFECSOL desc">
                    <SelectParameters>
