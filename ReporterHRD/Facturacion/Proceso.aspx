             ...
             <table style="width:30%;">
              ...
              <asp:GridView ID="GridView1" runat="server" AllowSorting="True" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" Font-Size="X-Small">
                  <Columns>
                      ...
                      <asp:BoundField DataField="Usuario_Egreso" HeaderText="Usuario_Egreso" ReadOnly="True" SortExpression="Usuario_Egreso" />
                      ...
                      <asp:BoundField DataField="USARIO_FACTURA" HeaderText="USARIO_FACTURA" SortExpression="USARIO_FACTURA" ReadOnly="True" />
                 ...
                 <caption>
                     <br />
                     <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DGEMPRES02ConnectionString %>" SelectCommand="SELECT DISTINCT ADNINGRESO.AINCONSEC AS Ingreso, ADNINGRESO.AINFECING AS Fecha_Ingreso, GENUSUARIO_1.USUDESCRI AS Usuario_Ingreso, CASE WHEN ADNINGRESO.AINURGCON &lt; 0 THEN 'Ninguno' WHEN ADNINGRESO.AINURGCON = 0 THEN 'Urgencias' WHEN ADNINGRESO.AINURGCON = 1 THEN 'Cons Externa' WHEN ADNINGRESO.AINURGCON = 2 THEN 'Nacido HRD' WHEN ADNINGRESO.AINURGCON = 3 THEN 'Remitido' WHEN ADNINGRESO.AINURGCON = 4 THEN 'Hosp Urg' WHEN ADNINGRESO.AINURGCON = 5 THEN 'Hospitalización' WHEN ADNINGRESO.AINURGCON &gt; 9 THEN 'Cirugia' END AS Ingreso_Por, CASE WHEN ADNINGRESO.AINTIPING = 1 THEN 'Ambulatorio' WHEN ADNINGRESO.AINTIPING = 2 THEN 'Hospitalario' END AS Clase_Ingreso, ADNINGRESO.AINFECHOS AS Fech_Hospitalización, (SELECT HCECONSEC FROM HCNEPICRI WHERE (ADNINGRESO = ADNINGRESO.OID)) AS Epicirisis, (SELECT HCEFECDOC FROM HCNEPICRI AS HCNEPICRI_2 WHERE (ADNINGRESO = ADNINGRESO.OID)) AS Fecha_Epicirisis, (SELECT GMENOMCOM FROM GENMEDICO WHERE (OID = (SELECT GENMEDICO FROM HCNEPICRI AS HCNEPICRI_1 WHERE (ADNINGRESO = ADNINGRESO.OID)))) AS MED_Epicirisis, (SELECT ADENUMEGR FROM ADNEGRESO WHERE (ADNINGRESO.ADNEGRESO = OID)) AS Egreso, (SELECT ADEFECSAL FROM ADNEGRESO AS ADNEGRESO_2 WHERE (ADNINGRESO.ADNEGRESO = OID)) AS Fecha_Egreso, (SELECT USUDESCRI FROM GENUSUARIO WHERE (OID = (SELECT GEENUSUARIO FROM ADNEGRESO AS ADNEGRESO_1 WHERE (ADNINGRESO.ADNEGRESO = OID)))) AS Usuario_Egreso, (SELECT TOP (1) SFANUMFAC FROM SLNFACTUR WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC) AS FACTURA, CAST((SELECT SUM(SLNSERPRO.SERVALPRO * SLNSERPRO.SERCANTID) AS Expr1 FROM SLNSERPRO INNER JOIN ADNINGRESO AS INGRESO ON SLNSERPRO.ADNINGRES1 = INGRESO.OID INNER JOIN SLNORDSER ON SLNSERPRO.SLNORDSER1 = SLNORDSER.OID INNER JOIN GENPACIEN ON INGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN GENDETCON ON INGRESO.GENDETCON = GENDETCON.OID LEFT OUTER JOIN SLNSERHOJ RIGHT OUTER JOIN GENSERIPS ON SLNSERHOJ.GENSERIPS1 = GENSERIPS.OID ON SLNSERPRO.OID = SLNSERHOJ.OID LEFT OUTER JOIN SLNPROHOJ RIGHT OUTER JOIN INNMSUMPA ON SLNPROHOJ.OID = INNMSUMPA.SLNPROHOJ ON SLNSERPRO.OID = SLNPROHOJ.OID LEFT OUTER JOIN INNPRODUC ON INNMSUMPA.INNPRODUC = INNPRODUC.OID WHERE (INGRESO.OID = ADNINGRESO.OID) AND (SLNORDSER.SOSESTADO &lt; 2) AND (SLNSERPRO.SERAPLPRO = 0)) AS DECIMAL(10 , 0)) AS VALOR, (SELECT TOP (1) SFAFECFAC FROM SLNFACTUR AS SLNFACTUR_3 WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC) AS Fecha_FACTURA, (SELECT TOP (1) CASE WHEN SLNFACTUR_2.SFADOCANU = 0 THEN 'Ok' ELSE 'Anulada' END AS Expr1 FROM SLNFACTUR AS SLNFACTUR_2 WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC) AS ESTADO, ADNINGRESO.AINOBSERV AS Obseravaciones, (SELECT TOP (1) CASE WHEN SLNFACTUR_1.SFADOCANU = 0 THEN 'Genero ' + (SELECT GENUSUARIO.USUDESCRI FROM GENUSUARIO WHERE GENUSUARIO.OID = SLNFACTUR_1.GENUSUARIO1) ELSE 'Anulo ' + (SELECT GENUSUARIO.USUDESCRI FROM GENUSUARIO WHERE GENUSUARIO.OID = SLNFACTUR_1.GENUSUARIO2) END AS Expr1 FROM SLNFACTUR AS SLNFACTUR_1 WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC) AS USARIO_FACTURA, GENPACIEN_1.PACNUMDOC AS Documento, GENPACIEN_1.PACPRIAPE + N' ' + GENPACIEN_1.PACSEGAPE + N' ' + GENPACIEN_1.PACPRINOM + N' ' + GENPACIEN_1.PACSEGNOM AS Paciente, GENDETCON_1.GDENOMBRE + N' (' + GENDETCON_1.GDECODIGO + N')' AS Entidad FROM GENPACIEN AS GENPACIEN_1 INNER JOIN HCNFOLIO ON GENPACIEN_1.OID = HCNFOLIO.GENPACIEN INNER JOIN ADNINGRESO ON HCNFOLIO.ADNINGRESO = ADNINGRESO.OID INNER JOIN GENDETCON AS GENDETCON_1 ON ADNINGRESO.GENDETCON = GENDETCON_1.OID INNER JOIN GENESPECI ON HCNFOLIO.GENESPECI = GENESPECI.OID INNER JOIN GENMEDICO AS GENMEDICO_1 ON HCNFOLIO.GENMEDICO = GENMEDICO_1.OID INNER JOIN GENUSUARIO AS GENUSUARIO_1 ON ADNINGRESO.GEENUSUARIO = GENUSUARIO_1.OID WHERE (ADNINGRESO.AINFECING BETWEEN @f1 AND @f2 + ' 23:59:59') and ADNINGRESO.AINESTADO in (0,1,3) ORDER BY Ingreso_Por DESC">
                         <SelectParameters>
