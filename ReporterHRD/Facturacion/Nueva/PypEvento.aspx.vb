...
Partial Class Facturacion_PypEvento
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        '        DataPlanesBeneficios.ConnectionString = "DG_ConnectionString"
        '    Else
        '        DataPlanesBeneficios.ConnectionString = "DGEMPRES03ConnectionString"
        '    End If
...
Consulta = " SELECT   '891855438' AS NitHRD, '1523800664' AS CodHabilitacionIPS, GEENENTADM.ENTCODIGO AS CodigoEPS,  SLNFACTUR.SFANUMFAC AS NumFactura,   CONVERT (nvarchar(10), SLNFACTUR.SFAFECFAC, 103) AS FechaFactura,   CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC + SLNFACTUR.SFAVALPAC) AS ValorBruto,   CONVERT (decimal(11 , 0), SLNFACTUR.SFAVALPAC) AS ValCopago,   CASE WHEN SLNSERPRO.SERCOPCTA = 2 THEN CONVERT (decimal(11 , 0) , SLNSERPRO.SERVALPAC) ELSE 0 END AS ValModeradora,   CONVERT (decimal(11 , 0), 0) AS ValorDesc,   CONVERT (decimal(11 , 0), 0) AS ValorIva,   'No' as 'Multiusuario',  CASE WHEN GENPACIEN.PACTIPDOC = 1 THEN 'CC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 2 THEN 'CE' ELSE  CASE WHEN GENPACIEN.PACTIPDOC = 3 THEN 'TI' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 4 THEN 'RC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 5 THEN 'PA' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 6 THEN 'AS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 7 THEN 'MS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 10 THEN 'CN' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 12 THEN 'CD' ELSE 'AS' END END END END END END END END END AS TipoDocPac,"
Consulta = Consulta + " GENPACIEN.PACNUMDOC AS NumDocPac,  REPLACE(GENPACIEN.PACPRIAPE, 'Ñ', 'N') AS PacPriApel, REPLACE(GENPACIEN.PACSEGAPE, 'Ñ', 'N') AS PacSegApel, REPLACE(GENPACIEN.PACPRINOM, 'Ñ', 'N') AS PacPriNom, REPLACE(GENPACIEN.PACSEGNOM, 'Ñ', 'N') AS PacSegNom, GENPACIEN.GPAFECNAC as 'Fech_Nacimiento', STR(DATEDIFF(year, GENPACIEN.GPAFECNAC, GETDATE()), 4, 0) AS EdadPac,  CASE WHEN GENPACIEN.GPASEXPAC = 1 THEN 'M' ELSE CASE WHEN GENPACIEN.GPASEXPAC = 2 THEN 'F' ELSE 'X' END END AS Sexo,  CONVERT (nvarchar(10), ADNINGRESO.AINFECING, 103) AS FechaEntrada, CONVERT (nvarchar(5), ADNINGRESO.AINFECING, 114) AS HoraEntrada, CASE WHEN CONVERT (nvarchar(10) , ADNEGRESO_1.ADEFECSAL , 103) IS NULL THEN CONVERT (nvarchar(10) , ADNINGRESO.AINFECING , 103) ELSE CONVERT (nvarchar(10) , ADNEGRESO_1.ADEFECSAL , 103) END AS FechaSalida, CASE WHEN CONVERT (nvarchar(5) , ADNEGRESO_1.ADEFECSAL , 114) IS NULL THEN CONVERT (nvarchar(5) , ADNINGRESO.AINFECING , 114) ELSE CONVERT (nvarchar(5) , ADNEGRESO_1.ADEFECSAL , 114) END AS HoraSalida, CONVERT (nvarchar(10), SLNSERPRO.SERFECSER, 103) AS FechaProced, CONVERT (nvarchar(5), SLNSERPRO.SERFECSER, 114) AS HoraProced,  (SELECT TOP (1) GENDIAGNO_3.DIACODIGO        FROM ADNDIAEGR INNER JOIN ADNEGRESO ON ADNDIAEGR.ADNEGRESO = ADNEGRESO.OID INNER JOIN GENDIAGNO AS GENDIAGNO_3 ON ADNDIAEGR.DIACODIGO = GENDIAGNO_3.OID WHERE (ADNDIAEGR.Tipo = 5) AND (ADNEGRESO.ADNINGRESO = ADNINGRESO.OID)) AS DxP,   '' AS DxS1, '' AS DxS2,    CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC + SLNFACTUR.SFAVALPAC) AS ValorBruto,   CONVERT (decimal(11 , 0), SLNFACTUR.SFAVALPAC) AS ValCopago,    CASE WHEN SLNSERPRO.SERCOPCTA = 2 THEN CONVERT (decimal(11 , 0) , SLNSERPRO.SERVALPAC) ELSE 0 END AS ValModeradora,   CONVERT (decimal(11 , 0), 0) AS ValorDesc,   CONVERT (decimal(11 , 0), 0) AS ValorIva,   '0' AS CodAutorizacion,    CASE WHEN SIPCODCUP IS NULL THEN IPRCODALT ELSE SIPCODCUP END AS CodPresPrincipal,  RTRIM(REPLACE(REPLACE(SLNSERPRO.SERDESSER, '+', ''), 'Ñ', 'N')) AS DescrpProced,    CONVERT (decimal(11 , 0), SLNSERPRO.SERCANTID) AS Cantidad, CONVERT (decimal(11 , 0), SLNSERPRO.SERVALPRO) AS ValorUnitario,    CONVERT (decimal(11 , 0), SLNSERPRO.SERCANTID) * CONVERT (decimal(11 , 0), SLNSERPRO.SERVALPRO) AS ValorTotServicio,   CONVERT (decimal(11 , 0), 0) AS ValCopago,  CASE WHEN SLNSERPRO.SERCOPCTA = 2 THEN CONVERT (decimal(11 , 0) , SLNSERPRO.SERVALPAC) ELSE 0 END AS ValModeradora, CONVERT (decimal(11 , 0), 0) AS ValorDesc, 	  CONVERT (decimal(11 , 0), 0) AS ValorIva   "       
Consulta = Consulta + " FROM GENCONTRA INNER JOIN GENTERCER ON GENCONTRA.GENTERCER1 = GENTERCER.OID INNER JOIN ADNINGRESO INNER JOIN SLNFACTUR ON ADNINGRESO.OID = SLNFACTUR.ADNINGRESO INNER JOIN SLNSERPRO ON ADNINGRESO.OID = SLNSERPRO.ADNINGRES1 AND SLNFACTUR.GENDETCON = SLNSERPRO.GENDETCON1 INNER JOIN GENDETCON ON SLNFACTUR.GENDETCON = GENDETCON.OID ON GENCONTRA.OID = GENDETCON.GENCONTRA1 INNER JOIN GEENENTADM ON GENCONTRA.DGNENTADM1 = GEENENTADM.OID INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN SLNORDSER ON SLNSERPRO.SLNORDSER1 = SLNORDSER.OID LEFT OUTER JOIN ADNEGRESO AS ADNEGRESO_1 ON ADNINGRESO.OID = ADNEGRESO_1.ADNINGRESO LEFT OUTER JOIN SLNSERHOJ INNER JOIN GENSERIPS ON SLNSERHOJ.GENSERIPS1 = GENSERIPS.OID ON SLNSERPRO.OID = SLNSERHOJ.OID LEFT OUTER JOIN INNPRODUC INNER JOIN SLNPROHOJ ON INNPRODUC.OID = SLNPROHOJ.INNPRODUC1 ON SLNSERPRO.OID = SLNPROHOJ.OID    "


        'SELECT SUBSTRING(SLNFACTUR.SFANUMFAC, 1, 4) AS Prefijo, SUBSTRING(SLNFACTUR.SFANUMFAC, 5, 20) AS NumFactura, CASE WHEN TERTIPDOC = 1 THEN 'CC' ELSE (CASE WHEN TERTIPDOC = 2 THEN 'CE' ELSE CASE WHEN TERTIPDOC = 5 THEN 'PA' ELSE 'NI' END END) END AS TipoDocIps, '891855438' AS NitHRD, '1523800664' AS CodHabilitacionIPS, GEENENTADM.ENTCODIGO AS CodigoEPS, 2 AS CodigoCuenta, '' AS CodigoContrato, CONVERT (nvarchar(10), SLNFACTUR.SFAFECFAC, 103) AS FechaFactura, CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC + SLNFACTUR.SFAVALPAC) AS ValorBruto, CONVERT (decimal(11 , 0), SLNFACTUR.SFAVALPAC) AS ValCopago, CONVERT (decimal(11 , 0), 0) AS ValCopCompartido, CONVERT (decimal(11 , 0), 0) AS ValorIva, CONVERT (decimal(11 , 0), 0) AS ValorIco, CONVERT (decimal(11 , 0), 0) AS ValorModeradora, CONVERT (decimal(11 , 0), 0) AS ValorDesc, '' AS ConcepDesc, CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC) AS ValorNeto, '' AS Periodo, 2 AS CodRegional, 1 AS ClasOrigen, CASE WHEN AINURGCON = 1 THEN '1' ELSE CASE WHEN AINURGCON = 10 THEN '1' ELSE CASE WHEN (SELECT TOP (1) CONVERT (nvarchar(4) , GENARESER.GASCODIGO) AS Expr1 FROM HPNDEFCAM INNER JOIN HPNESTANC ON HPNDEFCAM.OID = HPNESTANC.HPNDEFCAM INNER JOIN HPNGRUPOS ON HPNDEFCAM.HPNGRUPOS = HPNGRUPOS.OID INNER JOIN HPNSUBGRU ON HPNDEFCAM.HPNSUBGRU = HPNSUBGRU.OID INNER JOIN GENARESER ON HPNSUBGRU.GENARESER = GENARESER.OID WHERE (CONVERT (nvarchar(4) , GENARESER.GASCODIGO) = N'7320') AND (HPNESTANC.ADNINGRES = ADNINGRESO.OID)) = '7320' THEN '2' ELSE '3' END END END AS TipoServPgp, '' AS TipoServPaf, '' AS Finconsulta, '' AS DiasTratamiento, CASE WHEN GENPACIEN.PACTIPDOC = 1 THEN 'CC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 2 THEN 'CE' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 3 THEN 'TI' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 4 THEN 'RC' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 5 THEN 'PA' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 6 THEN 'AS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 7 THEN 'MS' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 10 THEN 'CN' ELSE CASE WHEN GENPACIEN.PACTIPDOC = 12 THEN 'CD' ELSE 'AS' END END END END END END END END END AS TipoDocPac, GENPACIEN.PACNUMDOC AS NumDocPac, REPLACE(GENPACIEN.PACPRINOM, 'Ñ', 'N') AS PacPriNom, REPLACE(GENPACIEN.PACSEGNOM, 'Ñ', 'N') AS PacSegNom, REPLACE(GENPACIEN.PACPRIAPE, 'Ñ', 'N') AS PacPriApel, REPLACE(GENPACIEN.PACSEGAPE, 'Ñ', 'N') AS PacSegApel, STR(DATEDIFF(year, GENPACIEN.GPAFECNAC, GETDATE()), 4, 0) AS EdadPac, CASE WHEN GENPACIEN.GPASEXPAC = 1 THEN 'M' ELSE CASE WHEN GENPACIEN.GPASEXPAC = 2 THEN 'F' ELSE 'X' END END AS Sexo, CASE WHEN GPAESTADO = 2 THEN '0' ELSE '1' END AS Estado, CASE WHEN GPADISSEV = 1 THEN 'S' ELSE 'N' END AS Discapacidad, CASE WHEN IPRTIPPRO = 1 THEN 'I' ELSE CASE WHEN IPRTIPPRO = 2 THEN 'M' ELSE 'P' END END AS TipoPrestacion, CASE WHEN SIPCODCUP IS NULL THEN IPRCODALT ELSE SIPCODCUP END AS CodPresPrincipal, '' AS CodPresDetalle, RTRIM(REPLACE(SLNSERPRO.SERDESSER, '+', '')) AS DescrpProced, CONVERT (nvarchar(10), SLNSERPRO.SERFECSER, 103) AS FechaProced, CONVERT (nvarchar(5), SLNSERPRO.SERFECSER, 114) AS HoraProced, CONVERT (decimal(11 , 0), SLNSERPRO.SERCANTID) AS Cantidad, CONVERT (decimal(11 , 0), SLNSERPRO.SERVALPRO) AS ValorUnitario, '0' AS ValorCompartido, CASE WHEN SLNSERPRO.SERCOPCTA = 2 THEN CONVERT (decimal(11 , 0) , SLNSERPRO.SERVALPAC) ELSE 0 END AS ValModeradora, '0' AS ValCopago, CONVERT (decimal(11 , 0), SLNSERPRO.SERCANTID) * CONVERT (decimal(11 , 0), SLNSERPRO.SERVALPRO) AS ValorTotServicio, '0' AS CodAutorizacion, (SELECT TOP (1) GENDIAGNO_3.DIACODIGO FROM ADNDIAEGR INNER JOIN ADNEGRESO ON ADNDIAEGR.ADNEGRESO = ADNEGRESO.OID INNER JOIN GENDIAGNO AS GENDIAGNO_3 ON ADNDIAEGR.DIACODIGO = GENDIAGNO_3.OID WHERE (ADNDIAEGR.Tipo = 5) AND (ADNEGRESO.ADNINGRESO = ADNINGRESO.OID)) AS DxP, ADNEGRESO_1.ADETIPDIA AS TipoDiagnostico, '' AS DxS1, '' AS DxS2, CONVERT (nvarchar(10), ADNINGRESO.AINFECING, 103) AS FechaEntrada, CONVERT (nvarchar(5), ADNINGRESO.AINFECING, 114) AS HoraEntrada, CASE WHEN CONVERT (nvarchar(10) , ADNEGRESO_1.ADEFECSAL , 103) IS NULL THEN CONVERT (nvarchar(10) , ADNINGRESO.AINFECING , 103) ELSE CONVERT (nvarchar(10) , ADNEGRESO_1.ADEFECSAL , 103) END AS FechaSalida, CASE WHEN CONVERT (nvarchar(5) , ADNEGRESO_1.ADEFECSAL , 114) IS NULL THEN CONVERT (nvarchar(5) , ADNINGRESO.AINFECING , 114) ELSE CONVERT (nvarchar(5) , ADNEGRESO_1.ADEFECSAL , 114) END AS HoraSalida, INNPRODUC.IPRTIPPRO, GENSERIPS.SIPCODCUP, SLNSERPRO.OID AS Expr1, ADNINGRESO.OID AS Expr3, ADNINGRESO.AINCONSEC, GENDETCON.OID AS Expr2, SLNSERPRO.GENDETCON1, SLNSERPRO.SERCANTID FROM GENCONTRA INNER JOIN GENTERCER ON GENCONTRA.GENTERCER1 = GENTERCER.OID INNER JOIN ADNINGRESO INNER JOIN SLNFACTUR ON ADNINGRESO.OID = SLNFACTUR.ADNINGRESO INNER JOIN SLNSERPRO ON ADNINGRESO.OID = SLNSERPRO.ADNINGRES1 AND SLNFACTUR.GENDETCON = SLNSERPRO.GENDETCON1 INNER JOIN GENDETCON ON SLNFACTUR.GENDETCON = GENDETCON.OID ON GENCONTRA.OID = GENDETCON.GENCONTRA1 INNER JOIN GEENENTADM ON GENCONTRA.DGNENTADM1 = GEENENTADM.OID INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN SLNORDSER ON SLNSERPRO.SLNORDSER1 = SLNORDSER.OID LEFT OUTER JOIN ADNEGRESO AS ADNEGRESO_1 ON ADNINGRESO.OID = ADNEGRESO_1.ADNINGRESO LEFT OUTER JOIN SLNSERHOJ INNER JOIN GENSERIPS ON SLNSERHOJ.GENSERIPS1 = GENSERIPS.OID ON SLNSERPRO.OID = SLNSERHOJ.OID LEFT OUTER JOIN INNPRODUC INNER JOIN SLNPROHOJ ON INNPRODUC.OID = SLNPROHOJ.INNPRODUC1 ON SLNSERPRO.OID = SLNPROHOJ.OID WHERE (SLNFACTUR.SFADOCANU = 0) AND (SLNORDSER.SOSESTADO = 1) AND (SLNSERPRO.SERVALENT > 0) AND (SLNFACTUR.SFANUMFAC = N'HRDB0002273544') AND (SLNSERPRO.SERCANTID >= 0.5) ORDER BY Expr1

        'WHERE (SLNFACTUR.SFADOCANU = 0) AND (SLNORDSER.SOSESTADO = 1) AND (SLNSERPRO.SERVALENT > 0) AND (ADNINGRESO.AINCONSEC = 770033) ORDER BY Expr1"
        ...
        Consulta = Consulta + " AND (SLNFACTUR.SFADOCANU = 0) AND (SLNSERPRO.SERVALENT > 0) AND (SLNORDSER.SOSESTADO = 1) AND (SLNSERPRO.SERCANTID >= 0.5)  "
    ...
    Protected Sub ListMeses_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ListMeses.SelectedIndexChanged
        ...
        If ListSedes.SelectedValue = 0 Then
            DataPlanesBeneficios.ConnectionString = "Data Source=PRINCIPALNET;Initial Catalog=DGEMPRES02;Persist Security Info=True;User ID=reporteador;Password=reporteador2"
        Else
            DataPlanesBeneficios.ConnectionString = "Data Source=PRINCIPALNET;Initial Catalog=DGEMPRES03;Persist Security Info=True;User ID=reporteador;Password=reporteador2"
        End If
    ...
    Function GenerarArchivo(ByVal FiltroConsulta As String, ByVal FiltroMes As String, ByVal FiltroContrato As String, ByVal NomContrato As String) 'ByVal Generar As Boolean, ByVal Tipo As String)
        ...
        Dim vbConexion As String

        If ListSedes.SelectedValue = 0 Then
            vbConexion = "DG_ConnectionString"
        Else
            vbConexion = "DGEMPRES03ConnectionString"
        End If

        Dim Conexion As New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings(vbConexion).ToString())
    ...
    Protected Sub BtnPreliminar_Click(sender As Object, e As System.EventArgs) Handles BtnPreliminar.Click
        ...
        If ListSedes.SelectedValue = 0 Then
            DataGridMuestra.ConnectionString = "Data Source=PRINCIPALNET;Initial Catalog=DGEMPRES02;Persist Security Info=True;User ID=reporteador;Password=reporteador2"
        Else
            DataGridMuestra.ConnectionString = "Data Source=PRINCIPALNET;Initial Catalog=DGEMPRES03;Persist Security Info=True;User ID=reporteador;Password=reporteador2"
        End If
