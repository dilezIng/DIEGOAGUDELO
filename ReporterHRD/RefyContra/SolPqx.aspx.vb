...
Partial Class PaginaBase
    ...
    Protected Sub GridView1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GridView1.SelectedIndexChanged
        ...
        Com.Connection = timedate
        timedate.Open()
        SQL = "SELECT DISTINCT HCNSOLPQX.OID , GENDETCON.GDENOMBRE + N' (' + GENDETCON.GDECODIGO + N')' ,GENSERIPS.SIPCODIGO + N'-' + RTRIM(GENSERIPS.SIPNOMBRE) , case when  GENPACIEN.PACPRIAPE is null then ' '  else GENPACIEN.PACPRIAPE end + N' ' + case when  GENPACIEN.PACSEGAPE  IS NULL THEN '' ELSE   GENPACIEN.PACSEGAPE  END+ N' ' + case when GENPACIEN.PACPRINOM is null then ' ' else GENPACIEN.PACPRINOM end + N' ' + CASE WHEN GENPACIEN.PACSEGNOM IS NULL THEN '' ELSE GENPACIEN.PACSEGNOM END FROM GENPACIEN  INNER JOIN HCNFOLIO ON GENPACIEN.OID = HCNFOLIO.GENPACIEN   INNER JOIN HCNSOLPQX ON HCNFOLIO.OID = HCNSOLPQX.HCNFOLIO INNER JOIN ADNINGRESO ON HCNFOLIO.ADNINGRESO = ADNINGRESO.OID INNER JOIN GENSERIPS ON HCNSOLPQX.GENSERIPS = GENSERIPS.OID INNER JOIN GENDETCON ON ADNINGRESO.GENDETCON = GENDETCON.OID  WHERE  (HCNSOLPQX.OID =" & SOLPQX & ")"
        Com = New SqlCommand(SQL, timedate)
        Rs = Com.ExecuteReader()
        Rs.Read()
        Labeli.Text = Rs(0)
