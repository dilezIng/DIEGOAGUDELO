...
Partial Class PaginaBase
    ...
    Protected Sub BtnActualizar_Click(sender As Object, e As System.EventArgs) Handles BtnActualizar.Click
        ...
        Dim timd As New SqlConnection("Data Source=PRINCIPALNET;Initial Catalog=DGEMPRES02;Persist Security Info=True;User ID=dghnet;Password=netdinamica;")
            ...
            timd.Open()
            sq = "SELECT Top 1 HCNSOLEXA.OID FROM HCNSOLEXA   INNER JOIN ADNINGRESO ON HCNSOLEXA.ADNINGRESO=ADNINGRESO.OID  INNER JOIN GENSERIPS ON HCNSOLEXA.GENSERIPS=GENSERIPS.OID   INNER JOIN GENMANSER ON GENSERIPS.OID=GENMANSER.GENSERIPS1   INNER JOIN GENMANUAL ON GENMANSER.GENMANUAL1=GENMANUAL.OID   INNER JOIN GENMANTAR on GENMANSER.OID=GENMANTAR.GENMANSER1   INNER JOIN HCNFOLIO ON HCNSOLEXA.HCNFOLIO=HCNFOLIO.OID    INNER JOIN GENPACIEN ON HCNFOLIO.GENPACIEN = GENPACIEN.OID INNER JOIN GENDETCON ON ADNINGRESO.GENDETCON = GENDETCON.OID where GENMANSER.GENMANUAL1=4   AND (YEAR(GENMANTAR.SMTFECINI) = YEAR(GETDATE()))    and GENSERIPS.SIPESTADO=1   and (CONVERT (char(10), HCNSOLEXA.HCSFECSOL, 103) = '" & Inicio & "') AND (ADNINGRESO.AINURGCON != 1 ) and (HCNSOLEXA.HCSVREXTERN=0) AND (GENMANTAR.SMTVALSER>321000) And GENDETCON.GDENOMBRE like '%MEDISALUD%' order by HCNSOLEXA.OID asc"
        ...
        Dim Coma1234567 As New SqlCommand
        Coma123456.Connection = timd
            timd.Open()
            sq = "SELECT Top 1 HCNSOLEXA.OID FROM HCNSOLEXA   INNER JOIN ADNINGRESO ON HCNSOLEXA.ADNINGRESO=ADNINGRESO.OID  INNER JOIN GENSERIPS ON HCNSOLEXA.GENSERIPS=GENSERIPS.OID   INNER JOIN GENMANSER ON GENSERIPS.OID=GENMANSER.GENSERIPS1   INNER JOIN GENMANUAL ON GENMANSER.GENMANUAL1=GENMANUAL.OID   INNER JOIN GENMANTAR on GENMANSER.OID=GENMANTAR.GENMANSER1   INNER JOIN HCNFOLIO ON HCNSOLEXA.HCNFOLIO=HCNFOLIO.OID    INNER JOIN GENPACIEN ON HCNFOLIO.GENPACIEN = GENPACIEN.OID INNER JOIN GENDETCON ON ADNINGRESO.GENDETCON = GENDETCON.OID where GENMANSER.GENMANUAL1=4   AND (YEAR(GENMANTAR.SMTFECINI) = YEAR(GETDATE()))    and GENSERIPS.SIPESTADO=1   and (CONVERT (char(10), HCNSOLEXA.HCSFECSOL, 103) = '" & Inicio & "') AND (ADNINGRESO.AINURGCON != 1 ) and (HCNSOLEXA.HCSVREXTERN=0) AND (GENMANTAR.SMTVALSER>321000) And GENDETCON.GDENOMBRE like '%MEDISALUD%' order by HCNSOLEXA.OID desc"
        Coma1234567 = New SqlCommand(sq, timd)
        Rsa1983 = Coma1234567.ExecuteReader()
        If Rsa1983.Read() Then
                f = Rsa1983(0)
                Rsa1983.Close()
            End If
        ...
        While (i <= f)
            ...
            timedate.Open()
            SQL = "Select case when (SELECT top 1 HCNSOLEXA.OID  FROM HCNSOLEXA   INNER JOIN ADNINGRESO ON HCNSOLEXA.ADNINGRESO=ADNINGRESO.OID  INNER JOIN GENSERIPS ON HCNSOLEXA.GENSERIPS=GENSERIPS.OID   INNER JOIN GENMANSER ON GENSERIPS.OID=GENMANSER.GENSERIPS1   INNER JOIN GENMANUAL ON GENMANSER.GENMANUAL1=GENMANUAL.OID   INNER JOIN GENMANTAR on GENMANSER.OID=GENMANTAR.GENMANSER1   INNER JOIN HCNFOLIO ON HCNSOLEXA.HCNFOLIO=HCNFOLIO.OID    INNER JOIN GENPACIEN ON HCNFOLIO.GENPACIEN = GENPACIEN.OID INNER JOIN GENDETCON ON ADNINGRESO.GENDETCON = GENDETCON.OID where  (HCNSOLEXA.OID = " & i & ") AND ( ADNINGRESO.AINURGCON <> 1) and (HCNSOLEXA.HCSVREXTERN=0) AND (GENMANTAR.SMTVALSER>321000) and GENMANSER.GENMANUAL1=4 AND (YEAR(GENMANTAR.SMTFECINI) = YEAR(GETDATE()))   And GENDETCON.GDENOMBRE like '%MEDISALUD%' ) is null then 1 else 2 end"
