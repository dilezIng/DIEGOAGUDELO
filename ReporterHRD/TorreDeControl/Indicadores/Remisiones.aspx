<%@ Page Title="Remisiones Pendientes" Language="VB" MasterPageFile="~/PaginaMaestra.master" AutoEventWireup="false" CodeFile="Remisiones.aspx.vb" Inherits="Urgencias_Estancia" %>
...
<style type="text/css">
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
        ...
        <tr style="font-weight: bold; font-size: 20pt; color: #FFFFFF; background-image: url('../Images/Fondo01.jpg');">
            <td bgcolor="#7EC5FD"  class="auto-style1" colspan="6">Remisiones Pendientes<asp:Label ID="LabelFolio" runat="server" ></asp:Label></td>
        ...
        <tr >
            <td colspan="4" style="font-size: 9pt" >
                    ...
                    <Columns>
                        ...
                        <asp:BoundField DataField="(Dias)" HeaderText="(Dias)" ReadOnly="True" SortExpression="(Dias)" />
						 ...
						 <asp:BoundField DataField="(Min)" HeaderText="(Min)" ReadOnly="True" SortExpression="(Min)" />
							 <asp:BoundField DataField="Min Total" HeaderText="Min Total" ReadOnly="True" SortExpression="Min Total" />
                ...
                <asp:SqlDataSource ID="DataGridEstancia" runat="server" 
                    ...
                    SelectCommand="SELECT DISTINCT HPNGRUPOS.HGRCODIGO AS Cod_Grupo, HPNDEFCAM.HCACODIGO AS Cama, ADNINGRESO.AINCONSEC AS Ingreso, GENPACIEN.PACNUMDOC AS Documento, LTRIM(RTRIM(GENPACIEN.PACPRIAPE + N' ' + GENPACIEN.PACSEGAPE + N' ' + GENPACIEN.PACPRINOM + N' ' + GENPACIEN.PACSEGNOM)) AS NomPaciente, ABS(DATEDIFF(yy, GENPACIEN.GPAFECNAC, GETDATE())) AS Edad, HPNGRUPOS.HGRNOMBRE AS Nom_Grupo, HPNESTANC.HESFECING AS Fecha_Ingreso, HPNESTANC.HESFECSAL AS Fecha_Salida, ABS(DATEDIFF(MINUTE, HPNESTANC.HESFECING , HPNESTANC.HESFECSAL)/ 1440) AS '(Dias)',   ABS(DATEDIFF(MINUTE, HPNESTANC.HESFECING , HPNESTANC.HESFECSAL)/ 60 % 24) AS '(Horas)',     ABS(DATEDIFF(MINUTE, HPNESTANC.HESFECING , HPNESTANC.HESFECSAL)%60) AS '(Min)', ABS(DATEDIFF(MINUTE, HPNESTANC.HESFECING , HPNESTANC.HESFECSAL)) AS 'Min Total', 					(SELECT DIACODIGO FROM GENDIAGNO WHERE (OID = (SELECT TOP (1) GENDIAGNO FROM HCNDIAPAC WHERE (HCNFOLIO = (SELECT TOP (1) OID FROM HCNFOLIO AS HCNFOLIO_2 WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC))))) AS Dx FROM HPNESTANC INNER JOIN HPNDEFCAM ON HPNESTANC.HPNDEFCAM = HPNDEFCAM.OID INNER JOIN ADNINGRESO ON HPNESTANC.ADNINGRES = ADNINGRESO.OID INNER JOIN HCNFOLIO AS HCNFOLIO_1 ON ADNINGRESO.OID = HCNFOLIO_1.ADNINGRESO INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID INNER JOIN HPNGRUPOS ON HPNDEFCAM.HPNGRUPOS = HPNGRUPOS.OID WHERE (HPNESTANC.HESFECING BETWEEN @FechaInicial AND @FechaFinal + ' 23:59:59') AND (HPNDEFCAM.HPNGRUPOS = @GrupoCama) ORDER BY Cama"
