...
Partial Class Facturacion_PypEvento
    ...
    Private Function ConsultaSql(ByVal Filtro As String) As String
        ...
        Consulta = "SELECT  SUBSTRING(SLNFACTUR.SFANUMFAC, 1, 4) AS Prefijo, SUBSTRING(SLNFACTUR.SFANUMFAC, 5, 20) AS NumFactura, GENDETCON.GDENOMBRE as Entidad,  GENDETCON.GDECODIGO AS CodEntidad , GEENENTADM.ENTCODIGO AS CodigoEPS, CONVERT (nvarchar(10), SLNFACTUR.SFAFECFAC, 103) AS FechaFactura, CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC + SLNFACTUR.SFAVALPAC) AS ValorBruto, CONVERT (decimal(11 , 0), SLNFACTUR.SFAVALPAC) AS ValCopago, CONVERT (decimal(11 , 0), 0) AS ValCopCompartido, CONVERT (decimal(11 , 0), 0) AS ValorIva, CONVERT (decimal(11 , 0), 0) AS ValorIco, CONVERT (decimal(11 , 0), 0) AS ValorModeradora, CONVERT (decimal(11 , 0), 0) AS ValorDesc, CONVERT (decimal(11 , 0), SLNFACTUR.SFATOTFAC) AS ValorNeto,(SELECT TOP (1)  (SELECT GENUSUARIO.USUNOMBRE FROM GENUSUARIO WHERE GENUSUARIO.OID = SLNFACTUR_1.GENUSUARIO1)  FROM SLNFACTUR AS SLNFACTUR_1 WHERE (ADNINGRESO = ADNINGRESO.OID) ORDER BY OID DESC) AS USARIO_FACTURA  "

        Consulta = Consulta + "FROM GENCONTRA INNER JOIN GENTERCER ON GENCONTRA.GENTERCER1 = GENTERCER.OID INNER JOIN ADNINGRESO INNER JOIN SLNFACTUR ON ADNINGRESO.OID = SLNFACTUR.ADNINGRESO INNER JOIN GENDETCON ON ADNINGRESO.GENDETCON = GENDETCON.OID ON GENCONTRA.OID = GENDETCON.GENCONTRA1 INNER JOIN GEENENTADM ON GENCONTRA.DGNENTADM1 = GEENENTADM.OID INNER JOIN GENPACIEN ON ADNINGRESO.GENPACIEN = GENPACIEN.OID    "
        ...
        Consulta = Consulta + " AND (SLNFACTUR.SFADOCANU = 0) "
        'Consulta = Consulta + "AND (SLNSERPRO.SERVALENT > 0) "

        Consulta = Consulta + "ORDER BY NumFactura "
    ...
    Protected Sub BtnNumfacts_Click(sender As Object, e As System.EventArgs) Handles BtnNumfacts.Click
        ...
        '    vbFiltro = vbFiltro + "SUBSTRING(SLNFACTUR.SFANUMFAC, 5, 20) = '000" + Elementos(i).ToString + "' or "  HRD0000007135
        'Next
